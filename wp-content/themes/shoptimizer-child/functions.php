...
function shoptimizer_child_custom_compare_button() {
    ...
    if ( class_exists( 'YITH_Woocompare_Frontend_Premium' ) || class_exists( 'YITH_Woocompare_Frontend' ) ) {
        ...
        // Check if $yith_woocompare and its obj property are set before trying to access them
        if ( isset( $yith_woocompare ) && isset( $yith_woocompare->obj ) && method_exists( $yith_woocompare->obj, 'add_compare_link' ) ) {
...
=======
    ...
    if ( class_exists( 'YITH_Woocompare_Frontend' ) ) {
        if ( method_exists( 'YITH_Woocompare_Frontend', 'is_product_in_compare' ) ) {
        ...
        if ( method_exists( 'YITH_Woocompare_Frontend_Premium', 'is_product_in_compare' ) ) {
    ...
    if ( $is_in_compare ) {
        ...
        $compare_url = '#'; // Default if method not found
        if ( class_exists( 'YITH_Woocompare_Frontend' ) && method_exists( 'YITH_Woocompare_Frontend', 'get_compare_page_url' ) ) {
            $compare_url = YITH_Woocompare_Frontend::get_compare_page_url();
        } elseif ( class_exists( 'YITH_Woocompare_Frontend_Premium' ) && method_exists( 'YITH_Woocompare_Frontend_Premium', 'get_compare_page_url' ) ) {
             $compare_url = YITH_Woocompare_Frontend_Premium::get_compare_page_url();
        }
>>>>>>> ec5b462 (fix(yith-compare): Add defensive checks for undefined method error)
    }
