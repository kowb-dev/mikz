/**
 * MKX Live Search Initialization & Debug
...
(function($) {
    ...
    function findSearchInput() {
        ...
        for (let selector of selectors) {
            ...
            if ($input.length) {
                console.log('MKX Live Search: Found input with selector:', selector);
                return $input.first();
            }
        }
    ...
    function forceInit() {
        ...
        if ($input.parent().hasClass('mkx-search-wrapper')) {
            console.log('MKX Live Search: Already initialized');
            return;
        }

        console.log('MKX Live Search: Initializing...');
        ...
        console.log('MKX Live Search: Wrapper created', $wrapper.length);
        ...
        console.log('MKX Live Search: Successfully initialized');
    }
    ...
    function renderResults(data, term, $container) {
        ...
        if (data.categories && data.categories.length > 0) {
            ...
            data.categories.forEach(function(category) {
                ...
                html += `<a href="${url}" style="display: block; padding: 6px 8px; color: #0f172a; text-decoration: none; border-radius: 4px; font-size: 14px; transition: all 0.2s;" 
        ...
        if (data.products && data.products.length > 0) {
            ...
            html += `<a href="${searchUrl}" style="display: block; padding: 12px; text-align: center; color: #f57300; text-decoration: none; font-weight: 500; border-top: 1px solid #f1f5f9;">
    ...
    function renderProduct(product) {
        const stockText = product.in_stock ? '' : '<span style="color: #ef4444; font-size: 11px;">Нет в наличии</span>';
        
        return `
            <a href="${product.url}" style="display: flex; gap: 12px; padding: 8px; text-decoration: none; border-radius: 4px; transition: all 0.2s;"
               onmouseover="this.style.background='#f8fafc';"
               onmouseout="this.style.background='transparent';">
                <div style="flex-shrink: 0; width: 60px; height: 60px; border-radius: 4px; overflow: hidden; border: 1px solid #f1f5f9;">
                    <img src="${product.image}" alt="${escapeHtml(product.title)}" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
    ...
    $(document).ready(function() {
        console.log('MKX Live Search: DOM Ready');
        forceInit();
    });

    $(window).on('load', function() {
        console.log('MKX Live Search: Window Load');
        forceInit();
    });

    setTimeout(function() {
        console.log('MKX Live Search: Delayed Init');
        forceInit();
    }, 1000);
