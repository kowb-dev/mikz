			...
			if ( ! empty( $args['table_nm'] ) && ( 'postmeta' === $args['table_nm'] ) && ( ( ! empty( $args['col_nm'] ) ) && ( true === in_array( $args['col_nm'], $price_columns, true ) ) ) ) {
				switch ( $args['col_nm'] ) {
					case '_sale_price_dates_from':
						update_post_meta( $args['id'], '_sale_price_dates_from', ( ! empty( $args['value'] ) ) ? sa_get_utc_timestamp_from_site_date( $args['value'] . ' 00:00:00' ) : '' );
						break;
					case '_sale_price_dates_to':
						update_post_meta( $args['id'], '_sale_price_dates_to', ( ! empty( $args['value'] ) ) ? sa_get_utc_timestamp_from_site_date( $args['value'] . ' 23:59:59' ) : '' );
						break;
