...
class Smart_Manager {
	...
	*/
	...
	function add_admin_notices() {
		if ( ( ! empty( $_GET['page'] ) ) && ( defined('SMPRO') ) && ( SMPRO === true ) && ( 'sm-storeapps-plugins' === $_GET['page'] || ( ( 'smart-manager' === $_GET['page'] ) && ( ! empty( $_GET['landing-page'] ) && ( 'sm-about' ===  $_GET['landing-page'] ) ) ) ) ) {
			$this->add_feature_notice( 'link_to_view_product_edit_history' );
		}
	...
	*/
	 ...
	 * @return void
	 */
	public function add_feature_notice( $feature_key = '' ) {
		$description = _x( 'Access WooCommerce product edit history directly from the product\'s row actions on the Products admin list page, as well as from the Product Edit page.', 'admin notice description for new feature', 'smart-manager-for-wp-e-commerce' );

		if ( ( empty( $feature_key ) ) || ( empty( $description ) ) ) {
			return;
		}
		//Remove previous feature notice DB options.
		$this->clear_previous_feature_notices( array( 'ai_based_advanced_search_for_products' ) );
		...
		printf(
			'<div class="sm_feature_notice notice notice-%s is-dismissible" id="sm_%s_notice" style="display:block !important;"><p>🚀 <strong>%s</strong> %s </p></div>',
			'success',
